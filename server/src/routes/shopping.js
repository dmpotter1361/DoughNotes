...
// within the same `source` bucket instead of skipping duplicates. Free-text lines
// (no parseable quantity) de-dupe by name. `source` keeps the weekly-planner bucket
// ('planner') separate from manual/per-recipe items ('list') so the planner can be
// regenerated by replacing only its own items (see clearSource).
export function addLabels(userId, labels, source = 'list') {
  const findQty = db.prepare(
    'SELECT id, qty FROM shopping_items WHERE user_id = ? AND source = ? AND checked = 0 AND name = ? AND unit IS ?'
  );
  const findText = db.prepare(
    'SELECT id FROM shopping_items WHERE user_id = ? AND source = ? AND checked = 0 AND name = ? AND unit IS NULL'
  );
  const insert = db.prepare(
    'INSERT INTO shopping_items (user_id, source, label, qty, unit, name, category) VALUES (?, ?, ?, ?, ?, ?, ?)'
  );
  ...
  for (const raw of labels) {
    ...
    if (p.qty == null) {
      ...
      if (!p.name) { insert.run(userId, source, p.raw, null, null, null, p.category); added++; continue; }
      if (findText.get(userId, source, p.name)) continue;
      insert.run(userId, source, p.display || p.raw, null, null, p.name, p.category);
      added++;
      continue;
    }
    const existing = findQty.get(userId, source, p.name, p.unit);
    if (existing) {
      const qty = (existing.qty || 0) + p.qty;
      update.run(qty, formatLabel({ qty, unit: p.unit, display: p.display }), existing.id);
    } else {
      insert.run(userId, source, formatLabel(p), p.qty, p.unit, p.name, p.category);
      added++;
    }
  }
  return added;
...
// Remove all of a user's items in one bucket (used to replace planner-generated items).
export function clearSource(userId, source) {
  db.prepare('DELETE FROM shopping_items WHERE user_id = ? AND source = ?').run(userId, source);
}
